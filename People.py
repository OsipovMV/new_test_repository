...
class Game:
    def __init__(self):
        self.tk = Tk()
        self.tk.title('man hurries to the exit')
        ...
        self.canvas_widdth = 500
        self.bcg = ['background.gif', 'background2.gif']
        #Делаю рандомное появление фона
        self.bg = PhotoImage (file=random.choice(self.bcg))
        w = self.bg.width()
        h = self.bg.height()
        for x in range(0, 5):
            for y in range (0, 5):
...
def within_x (co1, co2):
    if (co1.x1 > co2.x1 and co1.x1 < co2.x2) \
            or (co1.x2 > co2.x2 and co1.x2 < co2.x2) \
...
def within_y (co1, co2):
    if (co1.y1 > co2.y1 and co1.y1 < co2.y2) \
            ...
            or (co2.y1 > co1.y1 and co2.x1 < co1.y2) \
...
class StickFigureSprite(Sprite):
    ...
    def move(self):
        ...
        if self.y > 0 and co.y2 >= self.game.canvas_height:
            ...
            self.y = 0
            top = False
        if self.x > 0 and co.x2 >= self.game.canvas_widdth:
            self.x = 0
            right = False
        ...
        for sprite in self.game.sprites:
            ...
            if right and self.x > 0 and collided_left(co, sprite_co):
                self.x = 0
                right = False
                if sprite.endgame:
                    self.game.running = False
